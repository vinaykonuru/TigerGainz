...
def get_matches(user_data_list, requests_list):
    ...
    for row in range(len(matching_df_request)):
        ...
        for column_index, column in enumerate(column_labels):
            if column == "days":
                rel_val = fuzz.partial_token_sort_ratio(matching_df_request.iloc[row][column_index-1], matching_df_user.iloc[0][column_index])
                ...
                rel_val = fuzz.ratio(matching_df_request.iloc[row][column_index-1], matching_df_user.iloc[0][column_index])
                # ranker = priorities.get(column_labels[column])
                # cut_off = reference_ranker.get(ranker)
                cut_off = int(preferences.index(column))
